...
if ( ! class_exists( 'Themify_Builder' ) ) {
	...
	class Themify_Builder {
		...
		function init() {
			...
			if ( ! Themify_Builder_Model::is_frontend_editor_page() ) {
				if ( isset( $_GET['themify_builder_infinite_scroll'] ) && 'yes' == $_GET['themify_builder_infinite_scroll'] ) {
					...
					add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_stylesheet' ), 12 );
		...
		function get_template_module( $mod, $builder_id = 0, $echo = true, $wrap = true, $class = null, $identifier = array() ) {
			...
			if ( Themify_Builder_Model::is_frontend_editor_page() || ( isset( $_GET['themify_builder_infinite_scroll'] ) && 'yes' == $_GET['themify_builder_infinite_scroll'] ) ) {
		...
		function test_and_enqueue() {
			...
			if ( $this->is_readable_and_not_empty( $stylesheet_path ) ) {
				...
				wp_enqueue_style( $handler['filename'] , $this->get_stylesheet( 'byurl' ), $this->get_dependencies(), $this->get_stylesheet_version() );
		...
		/**
		 * Return array with dependencies.
		 * 
		 * @since 2.2.5
		 */
		function get_dependencies() {
			if ( wp_style_is( 'themify-media-queries' ) ) {
				return array( 'themify-media-queries' );
			} elseif ( wp_style_is( 'theme-style' ) ) {
				return array( 'theme-style' );
			} elseif ( wp_style_is( 'themify-style' ) ) {
				return array( 'themify-style' );
			}
		}
