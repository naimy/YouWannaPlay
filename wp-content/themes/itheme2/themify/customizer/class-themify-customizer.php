...
class Themify_Customizer {
	...
	function test_and_enqueue() {
		if ( $this->is_readable_and_not_empty( $this->get_stylesheet() ) ) {
			wp_enqueue_style( 'themify-customize', $this->get_stylesheet( 'byurl' ), $this->get_dependencies(), $this->get_stylesheet_version() );
			$this->enqueue_fonts();
			return true;
		}
		return false;
	...
	function generate_css( $fonts_only = false ) {
		...
		foreach ( $this->styles as $selector => $style ) {
			...
			if ( isset( $style[0] ) ) {
				if ( is_array( $style[0] ) ) {
					foreach( array_unique( $style, SORT_REGULAR ) as $mstyle ) {
